...
import { Send, Sparkles, Phone, ShieldCheck } from 'lucide-react';
import { STARTER_PROMPTS } from '../data/guidelines';
...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  // Re-focus the textarea once loading finishes so user can easily keep typing
  useEffect(() => {
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    console.log('[PinkCare ChatInput] handleSubmit called:', {
      input: input.trim(),
      isLoading,
    });

    if (!input.trim() || isLoading) {
      console.warn('[PinkCare ChatInput] Submit skipped: empty input or isLoading true');
      return;
    }
  ...
  return (
    <div className="p-4 sm:p-6 border-t border-[#FCE2E4] bg-[#FFFBFB] shrink-0">
      <div className="max-w-4xl mx-auto space-y-3">
        {/* Suggested Quick Prompt Pills (when active chat) */}
        {showStarterPrompts && (
          <div className="space-y-1.5 animate-fadeIn">
            <div className="flex items-center gap-1.5 text-[11px] text-slate-400 font-bold uppercase tracking-wider px-1">
              <Sparkles className="w-3 h-3 text-[#D84C6F]" />
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none no-scrollbar">
              {STARTER_PROMPTS.slice(0, 4).map((prompt, idx) => (
                <button
                  ...
                  disabled={isLoading}
                  className="shrink-0 text-xs px-3 py-1.5 rounded-full bg-white hover:bg-[#FDF4F5] border border-[#FCE2E4] text-[#4A1D28] font-medium transition-all text-left disabled:opacity-50 disabled:cursor-not-allowed shadow-2xs"
                >
                  <span className="text-[#D84C6F] font-bold mr-1">
                    {prompt.category}:
                  </span>
                  {prompt.text}
                </button>
        ...
        {/* Input Bar */}
        <form onSubmit={handleSubmit} className="flex items-end bg-white border border-[#FCE2E4] rounded-2xl p-1.5 sm:p-2 shadow-xs focus-within:border-[#D84C6F] focus-within:ring-2 focus-within:ring-[#FCE2E4] transition-all">
          <textarea
            ref={textareaRef}
            value={input}
            onChange={(e) => setInput(e.target.value)}
            onKeyDown={handleKeyDown}
            placeholder="Ask a question about screening, symptoms, or terminology..."
            rows={1}
            disabled={isLoading}
            className="flex-1 px-3 sm:px-4 py-2 text-sm text-slate-800 placeholder:text-slate-400 outline-none bg-transparent resize-none max-h-32 disabled:opacity-60"
          />

          <button
            type="submit"
            disabled={!input.trim() || isLoading}
            className="bg-[#D84C6F] text-white px-5 sm:px-6 py-2.5 rounded-xl text-sm font-bold hover:bg-[#C23B5C] active:bg-[#A92E4E] disabled:bg-slate-200 disabled:text-slate-400 transition-colors shadow-2xs shrink-0 flex items-center justify-center gap-1.5 cursor-pointer disabled:cursor-not-allowed"
          >
            <span>Send</span>
        ...
        {/* Footer info & Trusted Source Badges */}
        <div className="flex flex-wrap items-center justify-between gap-2 px-1 text-[11px] text-slate-400">
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1 text-slate-500">
              ...
              <span>Grounded via cancer.org, cdc.gov, nci, who.int, nccn.org</span>
            </span>
          </div>

          <button
            onClick={onOpenHotline}
            className="text-[#D84C6F] hover:text-[#C23B5C] font-semibold hover:underline inline-flex items-center gap-1"
          >
            <Phone className="w-3 h-3" />
            <span>24/7 Helpline: 1-800-227-2345</span>
          </button>
        </div>
      </div>
    </div>
...
const DOMAIN_LABELS: Record<string, string> = {
  'cancer.org': 'cancer.org (American Cancer Society)',
  'cancer.gov': 'cancer.gov (National Cancer Institute)',
  'nccn.org': 'nccn.org (NCCN Clinical Guidelines)',
  'who.int': 'who.int (World Health Organization)',
  'cdc.gov': 'cdc.gov (Centers for Disease Control)',
  'mayoclinic.org': 'mayoclinic.org (Mayo Clinic)',
  'breastcancer.org': 'breastcancer.org Foundation',
};
...
export const CitationCard: React.FC<CitationCardProps> = ({ citations }) => {
  ...
  return (
    <div className="pt-4 border-t border-rose-100/70 text-xs">
      <div className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1.5">
        <Bookmark className="w-3 h-3 text-[#D84C6F]" />
        <span>Medical Sources & Grounded Citations</span>
      </div>

      <div className="space-y-1.5">
        {citations.map((cite, index) => {
          const domain = cite.domain || '';
          const knownOrg = Object.entries(DOMAIN_LABELS).find(([key]) =>
            domain.includes(key)
          );
          const orgLabel = knownOrg ? knownOrg[1] : domain || 'Medical Source Reference';
          ...
          return (
            <a
              ...
              className="bg-slate-50 hover:bg-[#FDF4F5] p-2.5 rounded-lg text-[11px] text-slate-700 border border-slate-200/80 hover:border-[#FCE2E4] flex items-center justify-between transition-all group shadow-2xs"
              title={cite.title || cite.url}
            >
              <div className="flex items-center gap-2 min-w-0 pr-2">
                <span className="w-2 h-2 bg-emerald-500 rounded-full shrink-0" />
                <span className="font-semibold text-[#4A1D28] truncate">
                  {orgLabel}
                </span>
                {cite.title && cite.title !== orgLabel && (
                  <span className="text-slate-500 truncate hidden sm:inline text-[10px]">
                    — {cite.title}
                  </span>
                )}
              </div>
              <ExternalLink className="w-3.5 h-3.5 text-slate-400 group-hover:text-[#D84C6F] shrink-0" />
            </a>
          );
        })}
      </div>
...
export const DisclaimerBanner: React.FC = () => {
  ...
  return (
    <div className="bg-[#FFF0F0] border-b border-[#FCE2E4] text-[#D84C6F] text-xs transition-all duration-200 shrink-0">
      <div className="max-w-6xl mx-auto px-4 py-2 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-center sm:text-left flex-1 justify-center sm:justify-start">
          <div className="w-2 h-2 rounded-full bg-[#D84C6F] shrink-0" />
          <p className="font-semibold tracking-tight text-[12px]">
            This assistant provides general information only and does not provide medical diagnoses or advice.
          </p>
        </div>

        <button
          onClick={() => setIsExpanded(!isExpanded)}
          className="text-[11px] text-[#D84C6F] hover:text-[#C23B5C] font-semibold flex items-center gap-1 shrink-0 px-2 py-0.5 rounded hover:bg-white/60 transition-colors"
      ...
      {isExpanded && (
        <div className="bg-white/80 border-t border-[#FCE2E4] px-4 py-3 text-[#4A1D28] text-xs leading-relaxed max-w-6xl mx-auto space-y-1.5 animate-fadeIn">
          <p>
            • <strong>No Medical Diagnosis:</strong> We cannot evaluate whether a personal symptom is benign or serious. Always consult a licensed healthcare professional.
          </p>
          <p>
            • <strong>Evidence-Based Sources:</strong> Grounded strictly in medical consensus from the American Cancer Society, National Cancer Institute, NCCN, WHO, and CDC.
          </p>
          <p>
            • <strong>Emergency Care:</strong> If you are experiencing sudden severe pain, high fever, or immediate crisis, please call <strong>911</strong> or <strong>988</strong> right away.
          </p>
        </div>
      )}
    </div>
...
import React from 'react';
import { X, BookOpen, Calendar, HelpCircle, FileText, CheckCircle } from 'lucide-react';
import { SCREENING_GUIDELINES } from '../data/guidelines';
...
export const GuidelinesModal: React.FC<GuidelinesModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs animate-fadeIn">
      <div
        className="bg-white rounded-2xl shadow-xl border border-[#FCE2E4] max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-scaleUp"
        role="dialog"
        aria-modal="true"
        aria-labelledby="guidelines-title"
      >
        {/* Header */}
        <div className="bg-[#FDF4F5] px-6 py-4 border-b border-[#FCE2E4] flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-[#D84C6F] text-white flex items-center justify-center shadow-xs">
              <BookOpen className="w-5 h-5" />
            </div>
            <div>
              <h2 id="guidelines-title" className="text-lg font-bold text-[#4A1D28]">
                Official Screening Guidelines
              </h2>
              <p className="text-xs text-slate-600">
                Consensus recommendations by ACS, USPSTF, and NCCN
              </p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg text-slate-500 hover:text-[#4A1D28] hover:bg-white/80 transition-colors"
        ...
        <div className="p-6 overflow-y-auto space-y-5 text-slate-800 text-sm">
          ...
          <div className="space-y-3">
            <h3 className="font-bold text-[#4A1D28] flex items-center gap-2 text-sm">
              <Calendar className="w-4 h-4 text-[#D84C6F]" />
              <span>Age-Based Screening Milestones (Average Risk)</span>
            </h3>

            <div className="space-y-2.5">
              {SCREENING_GUIDELINES.map((item, idx) => (
                <div
                  key={idx}
                  className="p-3.5 rounded-xl bg-[#FDF4F5]/60 border border-[#FCE2E4] hover:border-[#D84C6F]/50 transition-colors"
                >
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <span className="font-bold text-[#4A1D28] text-sm">
                      {item.ageGroup}
                    </span>
                    <span className="text-[10px] text-[#D84C6F] font-bold bg-[#FFF0F0] px-2 py-0.5 rounded border border-[#FCE2E4]">
                      {item.source}
                    </span>
                  </div>
          ...
          {/* Mammogram Tips */}
          <div className="p-4 rounded-xl bg-[#FFF0F0] border border-[#FCE2E4] space-y-2">
            <h4 className="font-bold text-[#4A1D28] text-xs flex items-center gap-1.5">
              <FileText className="w-4 h-4 text-[#D84C6F]" />
              <span>Preparation for Screening Mammography</span>
            </h4>
            <ul className="text-xs text-slate-700 space-y-1.5 list-disc list-inside leading-relaxed">
              <li>
                Avoid deodorants, antiperspirants, powders, or lotions under arms or on breasts on the exam day (these can cause microcalcification shadows).
              </li>
              <li>
                If you have prior mammogram films/discs from another facility, bring or transfer them for historical comparison.
              </li>
              <li>
                Schedule the appointment 1 week after your menstrual cycle starts when breast tissue is least sensitive.
              </li>
            </ul>
          </div>
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
            <h4 className="font-bold text-[#4A1D28] text-xs flex items-center gap-1.5">
              <HelpCircle className="w-4 h-4 text-[#D84C6F]" />
              <span>Questions to Bring to Your Doctor</span>
            </h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-700">
              <div className="flex items-start gap-1.5">
                <CheckCircle className="w-3.5 h-3.5 text-emerald-600 shrink-0 mt-0.5" />
                <span>What is my personalized breast cancer risk profile?</span>
              </div>
              <div className="flex items-start gap-1.5">
                <CheckCircle className="w-3.5 h-3.5 text-emerald-600 shrink-0 mt-0.5" />
                <span>Do I have dense breast tissue, and should I get 3D Tomosynthesis?</span>
              </div>
              <div className="flex items-start gap-1.5">
                <CheckCircle className="w-3.5 h-3.5 text-emerald-600 shrink-0 mt-0.5" />
                <span>At what frequency should I schedule routine mammograms?</span>
              </div>
              <div className="flex items-start gap-1.5">
                <CheckCircle className="w-3.5 h-3.5 text-emerald-600 shrink-0 mt-0.5" />
                <span>Does my family history warrant genetic or BRCA screening?</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Footer */}
        <div className="px-6 py-3 bg-[#FDF4F5] border-t border-[#FCE2E4] flex items-center justify-between text-xs text-slate-500">
          <span>Sources: cancer.org, nccn.org, cdc.gov</span>
          <button
            onClick={onClose}
            className="px-4 py-1.5 bg-white border border-[#FCE2E4] hover:bg-slate-100 text-[#4A1D28] font-bold rounded-lg transition-colors"
          >
            Done
          </button>
        </div>
...
import React from 'react';
import { Heart, PhoneCall, BookOpen, RotateCcw, ShieldCheck, Menu } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-white/95 backdrop-blur-md border-b border-[#FCE2E4] sticky top-0 z-30 shadow-2xs">
      <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-2 sm:gap-4">
        {/* Mobile Menu & Brand */}
        <div className="flex items-center gap-3">
          <button
            onClick={onToggleMobileSidebar}
            className="md:hidden p-2 rounded-lg text-[#4A1D28] hover:bg-[#FDF4F5] border border-[#FCE2E4] transition-colors"
            aria-label="Toggle navigation menu"
          >
            <Menu className="w-5 h-5 text-[#D84C6F]" />
          </button>

          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-[#D84C6F] flex items-center justify-center shadow-xs text-white">
              <Heart className="w-4 h-4 fill-white/20" />
            </div>

            <div>
              <div className="flex items-center gap-1.5">
                <h1 className="font-bold text-[#4A1D28] text-base sm:text-lg leading-tight tracking-tight">
                  Pink Care AI
                </h1>
                <span className="text-[10px] font-bold text-[#D84C6F] bg-[#FFF0F0] border border-[#FCE2E4] px-2 py-0.5 rounded-full">
                  Informational
                </span>
              </div>
              <p className="text-[11px] text-slate-500 hidden sm:flex items-center gap-1">
                <ShieldCheck className="w-3.5 h-3.5 text-emerald-600 inline" />
                <span>Grounded in ACS, NCCN, NCI, CDC & WHO consensus</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-[#4A1D28] bg-[#FDF4F5] hover:bg-white border border-[#FCE2E4] rounded-lg transition-colors shadow-2xs"
            title="View Screening Guidelines"
          >
            <BookOpen className="w-3.5 h-3.5 text-[#D84C6F]" />
          ...
          <button
            onClick={onOpenHotline}
            className="inline-flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-bold text-white bg-[#D84C6F] hover:bg-[#C23B5C] active:bg-[#A92E4E] rounded-lg transition-colors shadow-xs group"
            title="Connect with 24/7 Helplines & Live Specialists"
          >
            <PhoneCall className="w-3.5 h-3.5 text-white group-hover:scale-110 transition-transform" />
          ...
          {messageCount > 0 && (
            <button
              onClick={onResetChat}
              className="p-1.5 sm:px-2.5 sm:py-1.5 text-xs font-medium text-slate-500 hover:text-[#4A1D28] hover:bg-[#FDF4F5] rounded-lg border border-transparent hover:border-[#FCE2E4] transition-colors flex items-center gap-1"
...
import React from 'react';
import { X, Phone, HeartHandshake, ExternalLink, Clock, Sparkles } from 'lucide-react';
...
export const HotlineModal: React.FC<HotlineModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs animate-fadeIn">
      <div
        className="bg-white rounded-2xl shadow-xl border border-[#FCE2E4] max-w-xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-scaleUp"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="bg-[#FDF4F5] px-6 py-4 border-b border-[#FCE2E4] flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 id="hotline-title" className="text-lg font-bold text-[#4A1D28]">
                Talk to a Person / Helplines
              </h2>
              <p className="text-xs text-slate-600">
                Free, confidential live support specialists & peer navigators
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto space-y-4 text-slate-800 text-sm">
          {/* Support Banner */}
          <div className="p-3.5 bg-[#FFF0F0] rounded-xl border border-[#FCE2E4] text-xs text-[#4A1D28] flex items-start gap-2.5">
            <Sparkles className="w-4 h-4 text-[#D84C6F] shrink-0 mt-0.5" />
            <div>
              <span className="font-bold">You are never alone.</span> If you need emotional support, guidance regarding low-cost mammograms, or answers from a certified specialist, these resources are free and confidential.
            </div>
          </div>
              ...
              <div
                ...
                className={`p-4 rounded-xl border transition-all ${
                  hotline.isEmergency
                    ...
                    : 'bg-white border-[#FCE2E4] hover:border-[#D84C6F] shadow-2xs'
                }`}
              >
                <div className="flex items-start justify-between gap-3 mb-1.5">
                  <div>
                    <span className="font-bold text-[#4A1D28] block leading-tight">
                      {hotline.name}
                    </span>
                    <span className="text-xs text-slate-500">{hotline.organization}</span>
                  </div>
                  {hotline.badge && (
                    <span
                      className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full shrink-0 ${
                        hotline.isEmergency
                          ...
                          : 'bg-[#FFF0F0] text-[#D84C6F] border border-[#FCE2E4]'
                      }`}
                    >
                      {hotline.badge}
                    </span>
                  ...
                  <div className="flex items-center gap-2">
                    {hotline.url && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1 text-xs text-slate-600 hover:text-[#D84C6F] px-2.5 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors"
                      >
                        <span>Website</span>
                    ...
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 text-xs font-bold text-white bg-[#D84C6F] hover:bg-[#C23B5C] px-3 py-1.5 rounded-lg shadow-2xs transition-colors"
                    >
        ...
        {/* Footer */}
        <div className="px-6 py-3 bg-[#FDF4F5] border-t border-[#FCE2E4] flex items-center justify-between text-xs text-slate-500">
          <span>Always dial 911 for acute medical emergencies</span>
          <button
            onClick={onClose}
            className="px-4 py-1.5 bg-white border border-[#FCE2E4] hover:bg-slate-100 text-[#4A1D28] font-bold rounded-lg transition-colors"
          >
...
export const MessageItem: React.FC<MessageItemProps> = ({ message, onOpenHotline }) => {
  ...
  return (
    ...
    >
      {isUser ? (
        /* User Message Bubble */
        <div className="bg-rose-50 text-[#4A1D28] px-5 py-3.5 rounded-2xl rounded-tr-none border border-rose-100 max-w-[85%] sm:max-w-[70%] text-[14px] leading-relaxed shadow-sm">
          <p className="whitespace-pre-wrap">{message.text}</p>
        </div>
      ) : (
        /* Assistant Message */
        <div className="flex flex-col max-w-[95%] sm:max-w-[85%] space-y-2">
          {/* Header */}
          <div className="flex items-center space-x-2 px-1">
            <div className="w-6 h-6 bg-[#D84C6F] rounded-md flex items-center justify-center text-[10px] text-white font-bold shadow-2xs">
              AI
            </div>
          ...
          {/* Assistant Card Body */}
          <div className="bg-white border border-rose-100 p-5 sm:p-6 rounded-2xl rounded-tl-none shadow-md space-y-4 text-[15px] leading-relaxed text-slate-700">
            ...
            {message.isEmergency && (
              <div className="p-3.5 rounded-xl bg-amber-50 border border-amber-300 text-amber-950 flex flex-col gap-2">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <h4 className="font-bold text-xs text-amber-900">
                      Important: Urgent or Emergency Symptoms Detected
                    </h4>
                    <p className="text-xs text-amber-800 mt-0.5">
                      If you are experiencing acute severe pain, sudden worsening, high fever, or distress, please seek immediate medical attention or call 911 / 988.
                    </p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2 pt-1 border-t border-amber-200/80">
                  <a
                    href="tel:911"
                    className="inline-flex items-center gap-1 px-3 py-1 text-xs font-bold text-white bg-red-600 hover:bg-red-700 rounded-md transition-colors shadow-2xs"
                  >
                  ...
                  <a
                    href="tel:988"
                    className="inline-flex items-center gap-1 px-3 py-1 text-xs font-bold text-white bg-amber-700 hover:bg-amber-800 rounded-md transition-colors shadow-2xs"
                  >
                    <PhoneCall className="w-3 h-3" />
                  ...
                  <button
                    onClick={onOpenHotline}
                    className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold text-amber-900 bg-amber-100 hover:bg-amber-200 rounded-md transition-colors"
                  >
            ...
            {/* Markdown Text */}
            <div className="prose prose-sm max-w-none text-slate-700 prose-headings:text-[#4A1D28] prose-headings:font-bold prose-strong:text-[#4A1D28] prose-strong:font-semibold prose-a:text-[#D84C6F] prose-a:underline prose-ul:my-2 prose-li:my-0.5 space-y-3">
            ...
            {/* Mandatory / Symptom Disclaimer Footer Box */}
            <p className="text-[11px] italic text-slate-500 bg-rose-50/50 p-2.5 rounded-lg border border-rose-100/60">
              This isn't medical advice — please talk to a doctor about your specific situation.
            </p>
            ...
            <div className="flex items-center justify-end pt-1">
              <button
                onClick={handleCopy}
                className="inline-flex items-center gap-1 text-[11px] text-slate-400 hover:text-slate-700 px-2 py-1 rounded hover:bg-slate-50 transition-colors"
                title="Copy response"
              >
                {copied ? (
                  <>
...
import React from 'react';
import { BookOpen, PhoneCall, PlusCircle, Sparkles, Heart } from 'lucide-react';
import { STARTER_PROMPTS } from '../data/guidelines';
import { ChatMessage } from '../types';
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  // Extract user questions from message history if available
  ...
  return (
    <>
      ...
      {isOpenMobile && (
        <div
          className="fixed inset-0 bg-stone-900/40 backdrop-blur-2xs z-40 md:hidden"
          onClick={onCloseMobile}
        />
      )}

      <aside
        className={`w-72 bg-[#FDF4F5] border-r border-[#FCE2E4] flex flex-col shrink-0 z-40 transition-transform duration-200 ease-in-out md:static fixed inset-y-0 left-0 ${
          isOpenMobile ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
        }`}
      >
        {/* Brand Header */}
        <div className="p-5 border-b border-[#FCE2E4]/60">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-2.5">
              <div className="w-8 h-8 bg-[#D84C6F] rounded-lg flex items-center justify-center shadow-xs">
                <Heart className="w-4 h-4 text-white fill-white/20" />
              </div>
              <span className="text-xl font-bold text-[#4A1D28] tracking-tight">
                Pink Care AI
              </span>
            </div>
            {messages.length > 0 && (
              <button
                ...
                className="p-1.5 text-[#D84C6F] hover:bg-[#FCE2E4]/50 rounded-lg transition-colors text-xs flex items-center gap-1 font-semibold"
          ...
          <button
            ...
            className="w-full flex items-center justify-between px-3 py-2 text-xs font-semibold text-[#4A1D28] bg-white/80 hover:bg-white rounded-lg border border-[#FCE2E4] transition-colors shadow-2xs"
          >
            <span className="flex items-center gap-2">
              <BookOpen className="w-3.5 h-3.5 text-[#D84C6F]" />
              <span>Official Guidelines</span>
            </span>
            <span className="text-[10px] text-[#D84C6F] font-bold">ACS & WHO</span>
          </button>
        </div>

        {/* Navigation & History Section */}
        <div className="flex-1 p-5 overflow-y-auto space-y-5">
          {/* History / Active Session Queries */}
          {userQueries.length > 0 && (
            <div>
              <div className="text-[11px] font-bold text-[#D84C6F] uppercase tracking-widest mb-2.5 opacity-80">
                Current Session
              </div>
              <div className="space-y-1.5">
                {userQueries.slice(-4).map((query, i) => (
                  <div
                    key={i}
                    className="bg-white/70 p-2.5 rounded-lg border border-[#FCE2E4] text-xs text-[#4A1D28] font-medium truncate"
                    title={query}
                  >
                    {query}
                  </div>
          ...
          {/* Quick Topics */}
          <div>
            <div className="text-[11px] font-bold text-[#D84C6F] uppercase tracking-widest mb-2.5 opacity-80 flex items-center justify-between">
            ...
            <div className="space-y-1">
              {STARTER_PROMPTS.map((prompt, idx) => (
                <button
                  ...
                  className="w-full text-left p-2.5 rounded-lg text-xs text-slate-600 hover:text-[#D84C6F] hover:bg-white/50 transition-colors flex items-start gap-2 group"
                >
                  <span className="w-1.5 h-1.5 rounded-full bg-[#D84C6F]/40 group-hover:bg-[#D84C6F] mt-1.5 shrink-0 transition-colors" />
        ...
        <div className="mt-auto p-5 border-t border-[#FCE2E4] bg-[#FCE7E9]/30 space-y-2.5">
          <div className="text-[13px] font-bold text-[#4A1D28] leading-tight">
            Need to talk to someone?
          </div>
          <p className="text-[11px] text-slate-600 leading-relaxed">
            Connect with a free confidential support specialist for emotional or clinical guidance.
          </p>

          <div
            onClick={onOpenHotline}
            className="bg-white rounded-lg p-3 border border-[#FCE2E4] shadow-xs cursor-pointer hover:border-[#D84C6F] transition-all group"
          >
            <div className="flex items-center justify-between text-[10px] text-slate-400 uppercase font-bold mb-0.5">
              <span>24/7 Cancer Support</span>
              <PhoneCall className="w-3 h-3 text-[#D84C6F] group-hover:scale-110 transition-transform" />
            </div>
            <a
              href="tel:18002272345"
              onClick={(e) => e.stopPropagation()}
              className="text-[#D84C6F] font-bold text-sm hover:underline block"
            >
              1-800-227-2345
            </a>
          </div>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 sm:py-12 space-y-8 animate-fadeIn">
      ...
      <div className="text-center space-y-3">
        <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-[#FFF0F0] border border-[#FCE2E4] text-[#D84C6F] shadow-xs mb-1">
          <Heart className="w-7 h-7 fill-[#D84C6F]/20 text-[#D84C6F]" />
        </div>
        <h2 className="text-2xl sm:text-3xl font-bold text-[#4A1D28] tracking-tight">
          Pink Care AI Assistant
        </h2>
        <p className="text-sm sm:text-base text-slate-600 max-w-xl mx-auto leading-relaxed">
          A trustworthy, compassionate medical information guide for breast health, screening timelines, terminology, and risk factors — backed by verified clinical consensus.
        </p>
      </div>

      {/* Feature Pillar Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-left">
        <div className="p-4 rounded-xl bg-white border border-[#FCE2E4] shadow-xs space-y-1.5">
          <div className="flex items-center gap-2 text-[#4A1D28] font-bold text-xs">
            <ShieldCheck className="w-4 h-4 text-emerald-600" />
            <span>Grounded Evidence</span>
          </div>
          <p className="text-xs text-slate-600 leading-relaxed">
            Prioritizes verified consensus from the American Cancer Society, NCI, CDC, WHO, and Mayo Clinic.
          </p>
        </div>
        ...
        <div className="p-4 rounded-xl bg-white border border-[#FCE2E4] shadow-xs space-y-1.5">
          ...
          <p className="text-xs text-slate-600 leading-relaxed">
            Understand age milestones, dense breast tissue reports, mammograms, and ultrasound procedures.
          </p>
        </div>
        ...
        <div className="p-4 rounded-xl bg-white border border-[#FCE2E4] shadow-xs space-y-1.5">
          <div className="flex items-center gap-2 text-[#4A1D28] font-bold text-xs">
            ...
            <span>No Diagnosis Policy</span>
          </div>
          <p className="text-xs text-slate-600 leading-relaxed">
            We do not evaluate or judge personal symptoms. We empower you with clear context to talk to your doctor.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between text-xs text-slate-400 font-bold uppercase tracking-wider px-1">
          <span>Common Topics & Questions</span>
          <button
            onClick={onOpenGuidelines}
            className="text-[#D84C6F] hover:text-[#C23B5C] font-semibold hover:underline flex items-center gap-1 normal-case text-xs"
          >
            <BookOpen className="w-3.5 h-3.5" />
            <span>Screening Milestones</span>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
          {STARTER_PROMPTS.map((prompt, idx) => (
            <button
              ...
              className="p-3.5 rounded-xl bg-white hover:bg-[#FDF4F5] border border-[#FCE2E4] text-left transition-all group shadow-2xs flex items-start gap-2.5 hover:border-[#D84C6F]"
            >
              <HelpCircle className="w-4 h-4 text-[#D84C6F] shrink-0 mt-0.5 group-hover:scale-110 transition-transform" />
              <div>
                <span className="text-[11px] font-bold text-[#D84C6F] uppercase tracking-wide block">
                  {prompt.category}
                </span>
                <span className="text-xs text-[#4A1D28] font-medium group-hover:text-[#D84C6F] transition-colors">
                  {prompt.text}
                </span>
              </div>
            </button>
      ...
      {/* Support Card */}
      <div className="p-4 rounded-xl bg-[#FDF4F5] border border-[#FCE2E4] flex flex-col sm:flex-row items-center justify-between gap-3 text-center sm:text-left">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-[#D84C6F] text-white flex items-center justify-center shrink-0 shadow-xs">
            <PhoneCall className="w-5 h-5" />
          </div>
          <div>
            <h4 className="font-bold text-xs text-[#4A1D28]">
              Need to speak with a cancer support specialist?
            </h4>
            <p className="text-xs text-slate-600">
              Access free, confidential 24/7 helplines and peer volunteer navigators.
            </p>
          </div>
        </div>
        ...
        <button
          onClick={onOpenHotline}
          className="shrink-0 px-4 py-2 rounded-lg bg-[#D84C6F] hover:bg-[#C23B5C] text-white text-xs font-bold shadow-xs transition-colors"
        >
          View 24/7 Helplines
        </button>
      </div>
...
const STORAGE_KEY = 'pink_care_chat_history_v1';
...
export default function App() {
  ...
  const handleSendMessage = async (text: string) => {
    console.log('[PinkCare] handleSendMessage called with text:', text, {
      isLoading,
    ...
    if (!text || !text.trim()) {
      console.warn('[PinkCare] handleSendMessage aborted: empty text');
      return;
    }

    if (isSendingRef.current) {
      console.warn('[PinkCare] handleSendMessage aborted: request already in progress');
      return;
    }
    ...
    // Filter out prior temporary error bubbles so clean conversation is sent to model
    const cleanHistory = messages.filter((m) => m.status !== 'error');
    const newHistory = [...cleanHistory, userMessage];

    setMessages(newHistory);

    const controller = new AbortController();
    const CLIENT_TIMEOUT_MS = 35000; // 35-second client-side fallback timeout
    const timeoutId = setTimeout(() => {
      controller.abort();
    }, CLIENT_TIMEOUT_MS);

    try {
      console.log('[PinkCare] Dispatching POST /api/chat with', newHistory.length, 'messages');
      ...
      console.log('[PinkCare] Received response from /api/chat:', {
        citationsCount: data.citations?.length || 0,
        hasDisclaimer: data.hasSymptomDisclaimer,
      });

      const assistantMessage: ChatMessage = {
        id: `model-${Date.now()}`,
        role: 'model',
        text: data.text || 'I could not generate an answer. Please consult a doctor.',
        timestamp: Date.now(),
        citations: data.citations || [],
        hasSymptomDisclaimer: data.hasSymptomDisclaimer,
      ...
      clearTimeout(timeoutId);
      console.error('[PinkCare] Chat error during /api/chat:', error);
      ...
      const rawErrorMessage = error?.message || 'Failed to connect to the assistant. Please try again.';
      const userDisplayError = isTimeout
        ? 'The request timed out after 35 seconds. Please try asking a shorter question or checking your connection.'
        : rawErrorMessage;

      setErrorMessage(userDisplayError);

      const errorMessageItem: ChatMessage = {
        id: `error-${Date.now()}`,
        role: 'model',
        text: isTimeout
          ? "The request timed out while processing. Please try asking again, or reach out directly to the American Cancer Society 24/7 Helpline at 1-800-227-2345 for immediate support."
          : `We encountered an issue processing your request (${rawErrorMessage}). If you have an urgent medical question or concern, please reach out to your doctor or contact the American Cancer Society 24/7 Helpline at 1-800-227-2345.`,
        timestamp: Date.now(),
        hasSymptomDisclaimer: true,
        status: 'error',
      };
      ...
      console.log('[PinkCare] handleSendMessage finished and reset isLoading to false');
    }
  };
  ...
  const handleResetChat = () => {
    if (
      messages.length > 0 &&
      window.confirm('Are you sure you want to clear your current conversation?')
    ) {
      setMessages([]);
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-[#FFFBFB] font-sans text-slate-800 overflow-hidden selection:bg-[#FFF0F0] selection:text-[#D84C6F]">
      {/* Top Professional Polish Disclaimer Banner */}
      <DisclaimerBanner />
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col bg-white overflow-hidden">
          ...
          {/* Conversation Stream */}
          <div className="flex-1 p-4 sm:p-6 md:p-8 overflow-y-auto space-y-4">
            {messages.length === 0 ? (
              <WelcomeHero
                onSelectPrompt={handleSendMessage}
                onOpenHotline={() => setIsHotlineOpen(true)}
              ...
              <div className="max-w-4xl mx-auto space-y-2">
                ...
                {/* Loading State */}
                {isLoading && (
                  <div className="flex items-center space-x-2 py-3 px-1 animate-fadeIn">
                    <div className="w-6 h-6 bg-[#D84C6F] rounded-md flex items-center justify-center text-[10px] text-white font-bold shadow-2xs">
                      <Loader2 className="w-3.5 h-3.5 animate-spin" />
                    </div>
                    <div className="bg-white border border-[#FCE2E4] px-4 py-2.5 rounded-xl rounded-tl-none text-xs text-slate-600 shadow-2xs flex items-center gap-2">
                      <span className="w-2 h-2 rounded-full bg-[#D84C6F] animate-pulse" />
                      <span>Consulting trusted consensus (cancer.org, cdc.gov, nci, who.int)...</span>
                    </div>
                  </div>
                )}
                ...
                {/* Error Banner */}
                {errorMessage && (
                  <div className="p-3 rounded-xl bg-red-50 border border-red-200 text-xs text-red-800 flex items-center gap-2 max-w-4xl mx-auto my-2">
...
!.env.example
{
  ...
  "packages": {
    ...
    "@google/genai": ["@google/genai@2.17.1", "", { "dependencies": { "google-auth-library": "^10.3.0", "p-retry": "^4.6.2", "protobufjs": "^7.5.4", "ws": "^8.18.0" }, "peerDependencies": { "@modelcontextprotocol/sdk": "^1.25.2" }, "optionalPeers": ["@modelcontextprotocol/sdk"] }, "sha512-CdZ3M/titoH81hXXkvOikrOW26bC9IXh9iYT7u+r+5p7wi1LnMEnB0AbJfDeWAkjuneP4oJ299BtCt6twmORWg=="],
...
<html lang="en">
  <head>
    ...
    <title>Pink Care AI Assistant - Breast Health & Cancer Information</title>
    <meta name="description" content="Informational breast health assistant backed by official medical consensus (ACS, NCCN, NCI, WHO, CDC)." />
    <meta property="og:title" content="Pink Care AI Assistant" />
    <meta property="og:description" content="A warm, trustworthy breast cancer informational assistant providing grounded guidance, screening information, and resources." />
  ...
  <body class="bg-stone-50 text-stone-800 antialiased font-['Plus_Jakarta_Sans',sans-serif]">
    <div id="root"></div>
...
{
  "name": "Pink Care AI Assistant",
  "description": "A warm, trustworthy breast cancer informational assistant providing grounded guidance, screening information, symptom overviews, and trusted medical resources.",
...
// Curated verified medical references mapped to topics
...
const VERIFIED_REFERENCES: MedicalReference[] = [
  {
    topicKeywords: ['mammogram', 'screening', 'age 40', 'age 45', 'age 50', 'how often', 'guideline', 'annual', 'frequency', 'ultrasound', 'mri'],
    title: 'ACS Recommendations for the Early Detection of Breast Cancer',
  ...
  {
    topicKeywords: ['symptom', 'lump', 'pain', 'discharge', 'dimple', 'nipple', 'swelling', 'redness', 'armpit', 'puckering', 'texture', 'rash'],
    title: 'CDC: What Are the Symptoms of Breast Cancer?',
    url: 'https://www.cdc.gov/breast-cancer/symptoms/',
    domain: 'cdc.gov',
    organization: 'Centers for Disease Control and Prevention',
  },
  {
    topicKeywords: ['dense', 'density', 'tissue', 'fibrocystic', 'dense breasts'],
    title: 'ACS: Breast Density and What It Means',
    url: 'https://www.cancer.org/cancer/types/breast-cancer/screening-tests-and-early-detection/breast-density.html',
    domain: 'cancer.org',
    organization: 'American Cancer Society',
  },
  {
    topicKeywords: ['biopsy', 'core needle', 'fine needle', 'diagnosis', 'calcification', 'microcalcification', 'birads', 'bi-rads'],
    title: 'ACS: Breast Biopsy Procedures & Understanding Results',
    url: 'https://www.cancer.org/cancer/types/breast-cancer/screening-tests-and-early-detection/breast-biopsy.html',
    domain: 'cancer.org',
    organization: 'American Cancer Society',
  },
  {
    topicKeywords: ['risk', 'genetic', 'brca', 'brca1', 'brca2', 'family history', 'hereditary', 'mutation'],
    title: 'NCI: Genetics of Breast and Gynecologic Cancers',
    url: 'https://www.cancer.gov/types/breast/patient/breast-genetics-pdq',
    domain: 'cancer.gov',
    organization: 'National Cancer Institute',
  },
  {
    topicKeywords: ['treatment', 'chemo', 'chemotherapy', 'radiation', 'surgery', 'mastectomy', 'lumpectomy', 'hormone therapy', 'tamoxifen', 'immunotherapy', 'stage'],
    title: 'NCI: Breast Cancer Treatment Options & Clinical Guidelines',
    url: 'https://www.cancer.gov/types/breast/patient/breast-treatment-pdq',
    domain: 'cancer.gov',
    organization: 'National Cancer Institute',
  },
  {
    topicKeywords: ['support', 'helpline', 'hotline', 'counseling', 'phone', 'help', 'talk to someone'],
    title: 'ACS 24/7 Cancer Helpline (1-800-227-2345)',
    url: 'https://www.cancer.org/about-us/online-help/helpline.html',
    domain: 'cancer.org',
    organization: 'American Cancer Society',
  },
];

const SYSTEM_INSTRUCTION = `You are "Pink Care AI Assistant", an informational assistant dedicated to breast health and breast cancer awareness.

Your role:
You answer questions about breast cancer symptoms, screening guidelines, risk factors, diagnostic procedures (e.g., mammograms, ultrasounds, biopsies), treatment types (e.g., surgery, radiation, chemotherapy, hormonal therapy), and medical terminology, strictly using well-established medical consensus (such as ACS, NCCN, NCI, WHO, and CDC guidelines).

Hard Rules & Guardrails:
1. NEVER DIAGNOSE: Never tell a user whether THEY personally have cancer, or evaluate how serious THEIR specific symptom is.
2. PERSONAL SYMPTOM INQUIRIES: If a user describes a personal symptom (such as a lump, unusual nipple discharge, skin dimpling/puckering, breast pain, redness, nipple inversion, or texture changes), DO NOT evaluate or judge it. Respond with genuine empathy, explain in general educational terms what that category of symptom can indicate, and clearly recommend scheduling an evaluation with a doctor, gynecologist, or breast clinic promptly. DO NOT reassure them that it is "probably nothing" or downplay their concern.
3. EMERGENCIES & CRISIS: If a user's message suggests a potential medical emergency (e.g., rapid acute swelling with high fever, severe unmanaged pain, acute signs of infection) or mental health crisis / suicidal ideation, IMMEDIATELY prioritize guidance to contact emergency services (911 in the US or local emergency number), seek urgent medical care, or reach the 988 Suicide & Crisis Lifeline before any other information.
4. NO SPECIFIC TREATMENT DOSAGE OR PERSONALIZED PLANS: Never prescribe or recommend a specific drug dosage, medication regimen, or individualized clinical plan.
5. MANDATORY SYMPTOM DISCLAIMER: Whenever a conversation touches on personal symptoms, potential symptoms, screening decisions, or personal health situations, ALWAYS append this exact sentence at the end of the response:
"This isn't medical advice — please talk to a doctor about your specific situation."
6. HONESTY & CERTAINTY: If you are not confident that an answer is medically accurate, explicitly state:
"I don't know, seek medical guidance" or explain that you cannot confirm the information with certainty and advise consulting a healthcare provider.
7. CITATIONS & ACCURACY: Cite and reference trusted organizations (ACS, NCI, CDC, NCCN, WHO, Mayo Clinic).

Tone and Style:
- Warm, compassionate, calm, clear, and reassuring without being overly casual or patronizing.
- Use clear bullet points and readable paragraphs so information is accessible and easy to understand for someone who may be anxious.
- For simple non-medical greetings or general app navigation questions, be warmly conversational and explain how you can help.`;

// Shared GenAI client with telemetry header
const getGenAIClient = () => {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    throw new Error('GEMINI_API_KEY is not set. Please configure it in your environment/secrets.');
  }
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    // Clean and validate contents for Gemini API (must alternate user and model roles)
    ...
    // If message was passed separately and wasn't already at the end of messages, append it
    if (message && message.trim()) {
      const lastMsg = rawMessages[rawMessages.length - 1];
      if (!lastMsg || lastMsg.text !== message.trim() || lastMsg.role !== 'user') {
        rawMessages.push({
          role: 'user',
          text: message.trim(),
        });
    ...
    // Ensure alternating user/model sequence and that first turn is user
    ...
    for (const msg of rawMessages) {
      if (contents.length === 0) {
        ...
        if (prev.role === msg.role) {
          // Merge text if duplicate consecutive role
          prev.parts[0].text += `\n\n${msg.text}`;
        } else {
    ...
    if (contents.length === 0) {
      contents.push({
        role: 'user',
        parts: [{ text: message?.trim() || 'Hello' }],
      });
    }

    const lastUserQuery = rawMessages.filter((m) => m.role === 'user').pop()?.text || message || '';
    console.log(`[PinkCare /api/chat] Incoming query: "${lastUserQuery.substring(0, 80)}" (total history: ${contents.length} turns)`);

    let response: any = null;
    let successfulModel = '';
    let groundingSucceeded = false;

    // List of candidate models to try in priority order
    const CANDIDATE_MODELS = ['gemini-3.1-flash-lite', 'gemini-3.6-flash', 'gemini-3.7-flash'];
    ...
    for (const model of CANDIDATE_MODELS) {
      try {
        console.log(`[PinkCare] Attempting generation with model: ${model}`);
        ...
        if (response && response.text) {
          ...
          console.log(`[PinkCare] Successfully generated response with ${model} in ${Date.now() - startTime}ms`);
          break;
        }
      } catch (err: any) {
        lastError = err;
        console.warn(`[PinkCare] Model ${model} failed (${err?.message || err}). Trying next candidate...`);
      }
    }

    if (!response || !response.text) {
      console.error('[PinkCare] All candidate models failed. Last error:', lastError);
      return res.status(500).json({
        error: lastError?.message || 'Unable to generate response at this time. Please try again.',
      });
    }

    const text = response.text || "I'm sorry, I couldn't generate a response. Please try asking again or speak with a healthcare provider.";

    // Extract dynamic citations if present in metadata
    const citationsMap = new Map<string, { title: string; url: string; domain?: string; snippet?: string }>();
    ...
    for (const chunk of rawChunks) {
      if (chunk.web && chunk.web.uri) {
        ...
        const title = chunk.web.title || domain || 'Medical Source Reference';
        if (!citationsMap.has(url)) {
          citationsMap.set(url, {
            title,
            url,
            domain: domain || 'Medical Resource',
            snippet: 'Verified Organization',
          });
        }
      }
    }

    // Match verified authoritative reference sources based on query and response text
    const combinedSearchText = (lastUserQuery + ' ' + text).toLowerCase();
    
    // Only attach medical reference cards if the query or response discusses medical/health topics
    const isMedicalQuery =
      combinedSearchText.includes('cancer') ||
      combinedSearchText.includes('breast') ||
      combinedSearchText.includes('mammogram') ||
      combinedSearchText.includes('screening') ||
      combinedSearchText.includes('lump') ||
      combinedSearchText.includes('symptom') ||
      combinedSearchText.includes('biopsy') ||
      combinedSearchText.includes('dense') ||
      combinedSearchText.includes('doctor') ||
      combinedSearchText.includes('stage') ||
      combinedSearchText.includes('treatment') ||
      combinedSearchText.includes('radiation') ||
      combinedSearchText.includes('chemo') ||
      combinedSearchText.includes('risk') ||
      combinedSearchText.includes('genetic');

    if (isMedicalQuery) {
      for (const ref of VERIFIED_REFERENCES) {
        const matches = ref.topicKeywords.some((keyword) => combinedSearchText.includes(keyword));
        if (matches && !citationsMap.has(ref.url)) {
          citationsMap.set(ref.url, {
            title: ref.title,
            url: ref.url,
            domain: ref.domain,
            snippet: ref.organization,
          });
        }
      }
    }

    const citations = Array.from(citationsMap.values()).slice(0, 4);

    // Detect if symptom disclaimer or emergency is present
    const lowerText = text.toLowerCase();
    const hasSymptomDisclaimer =
      lowerText.includes("this isn't medical advice") ||
      lowerText.includes('talk to a doctor') ||
      lowerText.includes('consult your doctor') ||
      lowerText.includes('see a healthcare provider');

    const isEmergency =
      lowerText.includes('911') ||
      lowerText.includes('emergency services') ||
      lowerText.includes('urgent care') ||
      lowerText.includes('988');

    return res.json({
      text,
      citations,
      hasSymptomDisclaimer,
      isEmergency,
      modelUsed: successfulModel,
      responseTimeMs: Date.now() - startTime,
    });
  } catch (error: any) {
    console.error('[PinkCare] Uncaught error in /api/chat:', error);
    return res.status(500).json({
      error: error?.message || 'An error occurred while communicating with the assistant.',
    });
  }
});
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`Pink Care AI Assistant running on http://0.0.0.0:${PORT}`);
  });
}
